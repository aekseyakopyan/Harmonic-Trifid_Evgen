...
def export_optimized():
    ...
    output_filename = "/opt/harmonic-trifid/Harmonic-Trifid_Evgen/full_parses_optimized_30mb.xlsx"
    ...
    try:
        ...
        # Select and truncate text to 400 chars to save space
        query = """
        SELECT 
            SUBSTR(text, 1, 400) as text_short, 
            source, 
            direction, 
            contact_link, 
            last_seen 
        ...
        # Rename column back for clarity in Excel
        df = df.rename(columns={'text_short': 'text'})

        print(f"Found {len(df)} records. Exporting to Excel (optimized)...")
