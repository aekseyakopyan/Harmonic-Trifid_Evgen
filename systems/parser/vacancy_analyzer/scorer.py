...
class VacancyScorer:
    ...
    SPAM_PATTERNS = [
        ...
        r"халява", r"раздача денег",
        # розыгрыш как задача ("проведение розыгрышей") — не спам; блокируем только промо-розыгрыши
        r"розыгрыш\s+(?:призов|подарков|айфона|денег)|участвуй\s+в\s+розыгрыш|выиграй\s+в\s+розыгрыш",
        ...
        "купить отзывы",
        r"event[- ]агентств",
        r"маркетингово(?:е|м)\s+агентств",  # агентства ищут штатных сотрудников с низкими ставками
        r"занимаюсь\s+посевами",
        r"сделаю\s+посевы",
        r"коротк(?:ие|их)\s+ролик(?:и|ов)",
        # reels/рилс убраны как hard-block: слово встречается в многопозиционных постах
        # где основная вакансия — таргетолог/контекстолог. Блокируем только прямой найм рилсмейкера.
        r"(?:ищу|нужен|требуется|ищем)\s+(?:рилс[- ]?мейкер|reels[- ]?maker|reels[- ]?creator)",
        ...
        # Крипто/USDT скамы (98k+ сообщений в БД)
        ...
        r"антигруп|antgroup_pay|dd18898",
        r"крипт\w+.{0,30}(?:заработ|доход|инвест)",
        # MLM / скам-вакансии
        r"без\s+вложений.{0,50}(?:заработ|доход)",
        r"сетевой\s+(?:маркетинг|бизнес)",
        r"млм|мlm",
        r"удаленная\s+работа.{0,30}от\s+1[4-9]\+?\s*лет",
        r"(?:пассивный|пассивн\w+)\s+доход.{0,30}(?:от\s+\d+|без\s+вложений)",
        # Массовые рассылки
        r"рассылка\s+по\s+(?:чатам|группам)",
        r"твоё\s+сообщение\s+увидят\s+тысячи",
        r"@getclient_tg_bot",
        # Инфографика для маркетплейсов (350k+ сообщений в БД)
        r"\bинфографик[аеуи]\b.{0,30}(?:wb|wildberries|ozon|маркетплейс)",
        r"(?:главный\s+слайд|слайды).{0,20}\d+\s*(?:₽|руб)",
        r"\bкарточки?\b.{0,20}(?:wb|wildberries|ozon|озон|вб).{0,20}(?:оптимиз|дизайн|создам)",
        # Штатный найм: объявления о вакансии для менеджера/специалиста
        r"(?:💡|📌|🔥)?\s*вакансия[:\s]+(?:менеджер|директор|специалист|оператор|помощник|ассистент|маркетолог|продажник|контент)",
        r"набираем\s+команду\s+для\s+работы\s+с\s+\w",  # "набираем команду для работы с питомниками"
        # Штатный найм с зарплатой — не заказ услуги
        r"(?:зарплата|зп|з/п|оклад)\s+(?:от|до|:)?\s*\d+",
        r"(?:первый|1-й|испытательный)\s+месяц.{0,30}\d+",  # "первый месяц 30.000"
        # Ozon/WB ведение магазина — маркетплейс-менеджмент, не SEO/PPC
        r"ведение\s+магазин[аоы].{0,30}(?:ozon|озон|wb|wildberries|маркетплейс)",
        r"(?:ozon|озон|wb|wildberries)\s*[,.]?\s*(?:нужен|ищу|ведение|управление)",
        r"\bабс[- ]?анализ\b",  # ABC-анализ = маркетплейс-аналитика
        # Предложение работы (спам для сотрудников)
    ...
    OFFER_PATTERNS = [
        ...
        # "скину контакты/менеджера" — рекомендует чужое агентство, а не ищет услугу
        r"скину\s+(?:вам|тебе|им|его|её|их)?\s*(?:контакт|менеджера|специалиста|номер)",
        r"если\s+нужны\s+контакты",
        r"вел(?:а)?\s+(?:директ|контекст|продвижение).{0,40}(?:через|в|с)\s+агентств",
        ...
        # Глагол + сервис (с прилагательным между ними)
        r"\bнастрою\b.{0,25}\b(?:рекламу|директ|контекст|яндекс|google|авито|таргет)",
        r"\bсоздам\b.{0,25}\b(?:сайт|лендинг|дизайн|логотип)",
        r"\bзапущу\b.{0,25}\b(?:рекламу|директ|таргет|кампани)",
        r"\bгарантирую\b.{0,20}\b(?:результат|трафик|клиентов|заявки)",
        r"\bберу\b.{0,15}\b(?:проекты|заказы|проект)\b",
        r"\bпринимаю\s+заказы\b",
        r"\bоказываю\s+услуги\b",
        r"\b(?:расширяю|набираю)\s+(?:портфолио|кейсы|клиентов)",
        r"\bя\s+[А-Я][а-яё]+\s+[-–—]\s+(?:директолог|контекстолог|авитолог|таргетолог|сеошник|маркетолог)",
